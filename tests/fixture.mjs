...
export const asset = Object.freeze({ asset_id: 'asset:client-legacy', asset_version: '1.0.0', judgment_version: '1.0.0' });
...
export function payloadFor(count = 1) {
  ...
  return { profile: 'kdna.payload.judgment', profile_version: '0.2.0', asset,
    ...
    scope: { statement: 'Only the client HTTP fixture' }, judgments };
}
export function fixture(count = 1, mutate = () => {}, entries = [], mutateManifest = () => {}) {
  const payload = payloadFor(count);
  mutate(payload);
  const manifest = { format_version: '0.2.0', asset_id: asset.asset_id, asset_uid: 'asset:client-legacy-uid',
    asset_type: 'fixture', title: 'client synthetic fixture', version: '1.0.0', judgment_version: '1.0.0',
